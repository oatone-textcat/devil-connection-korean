...
class KoreanPatchInstaller(QMainWindow):
    ...
    def init_ui(self):
        ...
        # NOTE: The UI has several cards with minimum heights + generous margins.
        # If we lock the window size, the bottom log area can be clipped (especially on
        # high-DPI / display scaling). Use a minimum size + an initial size instead.
        ...
        credit_label = QLabel("프로그램 제작: Nyabi | 텍스트 및 폰트 적용: Oatone (팀 글냥이)")
        ...
        credit_label2 = QLabel("본문 번역: 체퓨 | 이미지 번역: 토니, 체퓨 | 영상 번역: 민버드")
        credit_label2.setFont(QFont(get_system_font(), 10))
        credit_label2.setAlignment(Qt.AlignmentFlag.AlignCenter)
        credit_label2.setStyleSheet("color: #4a5568; margin-top: 2px;")
        title_layout.addWidget(credit_label2)
        ...
        self.add_log("", "info")
        self.add_log("도움주신 분들:", "success")
        self.add_log("프로그램 제작: Nyabi", "success")
        self.add_log("텍스트 및 폰트 적용: Oatone (팀 글냥이)", "success")
        self.add_log("본문 번역: 체퓨, 이미지 번역: 토니, 체퓨 영상 번역: 민버드", "success")
        self.add_log("", "info")
